...
from modules.utils import calculate_hours, format_currency
...
class DoctorModule:
    ...
    def add_shift(self):
        ...
        try:
            cursor.execute("""
                INSERT INTO doctor_shifts (doctor_id, date, arrival_time, leave_time, hours_worked)
                VALUES (?, ?, ?, ?, ?)
            """, (self.current_doctor_id, date, arrival, leave, hours))
            conn.commit()
    ...
    def calculate_salary(self):
        ...
        cursor.execute("""
            ...
            WHERE doctor_id = ? AND strftime('%m', date) = ? AND strftime('%Y', date) = ?
